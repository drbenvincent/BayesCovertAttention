...
function [AUC, FAR, HR] = MCMCdetection(N, varT, varD, TRIALS)
% [AUC, FAR, HR] = MCMCdetection(2, 1, 4, 10000)
...
data.varD       = varD;
%%
% construct spatial prior, based on number of items
data.Lprior		= [prob_present.*(ones(data.N,1)/data.N) ; (1-prob_present)]';
...
for i=1:nchains
	initial_param(i).L			= data.N+1;
end

%%
% Call JAGS to generate MCMC samples
%sprintf('Step 1: Generating %d samples in JAGS... ',nsamples)
...
[samples, stats, structArray] = matjags( ...
    ...
    'monitorparams', {'L'}, ...     % List of latent variables to monitor
...
signal_trials	= dataset.L<=data.N;
noise_trials	= dataset.L==data.N+1;
...
for t=1:T
	...
	temp = vec( samples.L( [1:nchains] ,:,t) );
	
	% Calculate the distribution over L (for each trial) as generated by the
	% MCMC samples
	[Lfreq,n] = hist(temp,[1:data.N+1]);
	
	% Normalise into a probability disibution over L.
	Lprob=Lfreq./sum(Lfreq);
