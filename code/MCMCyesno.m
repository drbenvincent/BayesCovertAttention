...
for i=1:mcmcparams.generate.nchains
	initial_param(i).D			= randi(data.N+1);
end
...
% % visualise for N=2 case
% if N==2
% 	D = dataset.D;
% 	present_trials = D<=N
% 	figure(6), clf, hold on
% 	plot( data.x(1,present_trials==1), data.x(2,present_trials==1),'g.')
% 	plot( data.x(1,present_trials==0), data.x(2,present_trials==0),'r.')
% 	axis square
% 	drawnow
% end
...
for t=1:T
	...
	% MCMC samples
	[PosteriorD, n] = hist(temp,[1:data.N+1]);
...
S = Ppresent(signal_trials==1);
N = Ppresent(signal_trials==0);
